...
各プレーヤーの行には、表示中の9ホールの素点（数字のみ。記号ではなく数字表記）と、右端に HALF と TOTAL があります。

次のJSONだけを返してください（前後の説明・マークダウン・コードフェンス禁止）:
{
  "half": "OUT" または "IN",
  "hole_numbers": [表示されているホール番号9個, 例 10..18],
  "hole_pars": [各表示ホールのPAR9個],
  "players": [
    {"name_raw": "表示氏名", "scores": [9個の素点], "half_total": 数値, "grand_total": 数値}
  ]
}

厳守事項:
- scores は表示されている9マスのみ。まだ打っていない/空欄のマスは null。0で埋めない。
- 数字は素点そのまま。青字や色は無視して数値だけ読む。
- half は画面上部の "OUT"/"IN" 表示で判断する。
- 読めない値は null。推測で埋めない。
"""
